...
<button id="pay-crypto" class="p-6 rounded-2xl border border-slate-800 bg-slate-900/50 hover:border-emerald-500/50 text-left"><i class="fa-brands fa-ethereum text-emerald-400 text-2xl"></i><div class="font-bold mt-2">Support with Crypto</div><div class="text-xs text-slate-500">Direct to Web3 wallet — no custody</div></button>
<button id="pay-card" class="p-6 rounded-2xl border border-slate-800 bg-slate-900/50 hover:border-amber-500/50 text-left"><i class="fa-regular fa-credit-card text-amber-400 text-2xl"></i><div class="font-bold mt-2">Support with Card</div><div class="text-xs text-slate-500">Stripe → USDC to Web3 wallet</div></button>
</div>
<div class="mt-6 flex gap-2"><input id="amount" type="number" value="5" min="1" class="w-24 mono text-sm bg-slate-800 border border-slate-700 rounded-lg px-3 py-2"><span class="py-2 text-slate-500">USD</span></div>
...
if (!profile.notFound) {
  ...
  document.getElementById("pay-crypto")?.addEventListener("click", async () => {
    const amt = document.getElementById("amount").value || 5;
    document.getElementById("pay-status").textContent = "Opening wallet...";
    try { const tx = await pay.checkout({ amountUSD: Number(amt), method: "crypto" }); document.getElementById("pay-status").innerHTML = '<span class="text-emerald-300">✓ Sent: '+tx+'</span>'; } catch(e){ document.getElementById("pay-status").textContent = "✗ "+e.message; }
  });
  document.getElementById("pay-card")?.addEventListener("click", async () => {
    const amt = document.getElementById("amount").value || 5;
    document.getElementById("pay-status").textContent = "Redirecting to Stripe...";
    try { await pay.checkout({ amountUSD: Number(amt), method: "card" }); } catch(e){ document.getElementById("pay-status").textContent = "✗ "+e.message; }
  });
}
</script>
</body></html>`;
